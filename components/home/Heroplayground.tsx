...
const CARD_BG     = "rgba(20,22,28,0.97)";      // slightly lighter than pure black
const INNER_BG    = "rgba(12,14,18,0.95)";      // terminal / inner panels
...
function ConnectionLines({
  ...
  const d1 = curve(u.x + u.w, u.y + u.h / 2, t.x, t.y + t.h / 2);
  const d2 = curve(t.x + t.w, t.y + t.h / 2, p.x, p.y + p.h / 2);

  const lineColor  = dragging ? BLUE : "rgba(29,155,240,0.45)";
  const lineWidth  = dragging ? 1.8  : 1.2;
  const glowOpacity = dragging ? 0.5 : 0;
    ...
    <svg className="absolute inset-0 w-full h-full pointer-events-none overflow-visible">
      <defs>
        <marker id="arr" viewBox="0 0 10 10" refX="8" refY="5"
          ...
          <path d="M2 2L8 5L2 8" fill="none" stroke={lineColor}
      ...
      {dragging && (
        <>
          <path d={d1} fill="none" stroke={BLUE} strokeWidth="4"
            opacity="0.15" filter="url(#glow)" />
          <path d={d2} fill="none" stroke={BLUE} strokeWidth="4"
            opacity="0.15" filter="url(#glow)" />
        </>
      )}

      {/* main lines */}
      <path d={d1} fill="none" stroke={lineColor} strokeWidth={lineWidth}
        strokeDasharray="5 3.5" opacity={dragging ? 1 : 0.65}
        markerEnd="url(#arr)"
        style={{ transition: "stroke 0.2s, stroke-width 0.2s, opacity 0.2s" }}
      />
      <path d={d2} fill="none" stroke={lineColor} strokeWidth={lineWidth}
        strokeDasharray="5 3.5" opacity={dragging ? 1 : 0.65}
        markerEnd="url(#arr)"
        style={{ transition: "stroke 0.2s, stroke-width 0.2s, opacity 0.2s" }}
      />
...
function DraggableCard({
  ...
  const handleDrag = useCallback((_: unknown, info: { point: { x: number; y: number } }) => {
    ...
    onPosChange(id, x - el.offsetWidth / 2, y - el.offsetHeight / 2, el.offsetWidth, el.offsetHeight);
  ...
  return (
    <motion.div
      ...
      onDrag={handleDrag}
      onDragStart={() => { setDragging(true);  onDragChange(id);   }}
      onDragEnd  ={() => { setDragging(false); onDragChange(null); }}
      whileDrag={{ scale: 1.025, zIndex: 30 }}
      className="absolute cursor-grab active:cursor-grabbing select-none"
      style={{
        borderRadius: "10px",
        border: dragging ? `2px solid ${BLUE}` : `1.5px solid rgba(255,255,255,0.08)`,
        background: CARD_BG,
        boxShadow: dragging
...
function UserCard() {
  return (
    <div style={{ minWidth: 175 }} className="border-blue-400 border-dotted border-1 rounded-md" >
      {/* header */}
      ...
      <div className="flex items-center gap-3 px-3 py-3">
        ...
        <div className="flex flex-col gap-[3px]">
          ...
          <span className="text-[9px] tracking-[0.5px]" style={{ color: "rgba(255,255,255,0.35)" }}>
      ...
      <div className="flex flex-wrap gap-[5px] px-3 pb-3">
        {TAGS.map((t) => (
          <span key={t} className="text-[8px] px-[7px] py-[3px] tracking-[0.5px] rounded-full"
            style={{
              background: BLUE_DIM,
              color: BLUE,
...
function TerminalCard() {
  return (
    <div style={{ width: 280 }} className="border-blue-400 border-dotted border-1 rounded-md" >
      {/* macOS title bar */}
      <div className="flex items-center gap-[7px] px-[14px] py-[10px]"
        ...
        <div className="flex items-center gap-[5px]">
          <div className="w-[10px] h-[10px] rounded-full bg-[#ff5f57]" style={{ boxShadow: "0 0 4px #ff5f5788" }} />
          <div className="w-[10px] h-[10px] rounded-full bg-[#febc2e]" style={{ boxShadow: "0 0 4px #febc2e88" }} />
          <div className="w-[10px] h-[10px] rounded-full bg-[#28c840]" style={{ boxShadow: "0 0 4px #28c84088" }} />
        </div>
        <span className="text-[9px] tracking-[2px] uppercase mx-auto"
          style={{ color: "rgba(29,155,240,0.6)", fontFamily: "monospace" }}>
          bash — intekhab
        </span>
      </div>
        ...
        {TERMINAL_LINES.map((line, i) => (
          <div key={i} className="flex flex-col">
            <div className="flex items-start gap-[6px]">
              <span style={{ color: "#5a9e6a", opacity: 0.9 }}>❯</span>
          ...
          <div className="flex items-center gap-[5px]">
            <div className="w-[5px] h-[5px] rounded-full"
              style={{
                ...
                boxShadow: proj.status === "live"
                  ? "0 0 6px #22c55e88"
                  : "0 0 6px #f59e0b88",
              }} />
            <span className="text-[8px] tracking-[0.5px]"
...
/* ─── main ───────────────────────────────────────────── */
const INITIAL_POS: Record<CardId, Pos> = {
  user:     { x: 16,  y: 50,  w: 175, h: 140 },
  terminal: { x: 215, y: 20,  w: 280, h: 230 },
  product:  { x: 520, y: 60,  w: 175, h: 150 },
};

export default function HeroPlayground() {
  const constraintsRef = useRef<HTMLDivElement>(null);
  const [positions, setPositions] = useState<Record<CardId, Pos>>(INITIAL_POS);
  const [dragging, setDragging]   = useState<CardId | null>(null);

  const handlePosChange = useCallback((id: CardId, x: number, y: number, w: number, h: number) => {
  ...
  return (
    ...
    >
      ...
      {/* cards */}
      <DraggableCard id="user" initialX={20} initialY={60}
      ...
      <DraggableCard id="terminal" initialX={290} initialY={200}
        constraintsRef={constraintsRef}
        onDragChange={setDragging}
        onPosChange={handlePosChange}>
        <TerminalCard />
      ...
      <DraggableCard id="product" initialX={620} initialY={100}
        constraintsRef={constraintsRef}
        onDragChange={setDragging}
        onPosChange={handlePosChange}>
        <ProductCard />
